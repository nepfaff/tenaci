...
%final setpoint
    ...
    while i <= length(waypoints)
        if waypoints(i).groupToPrevious % Spline trajectory connecting a sequence of waypoints
           ...
           [setPointJointAngles, setPointTimes, failedPoints] = TaskSpaceCubicSplineTrajectorySetPoints(...
                splineWaypoints, timeForTrajectory, samplePeriod...
            );
        
            if failedPoints > 0
               % Try joint space spline instead
               fprintf("taskSpaceStagesFromWaypoints: Cubic spline interpolation " +...
                    "failed in task space, trying again in joint space\n");
               [setPointJointAngles, setPointTimes] = JointSpaceCubicSplineTrajectorySetPoints(...
                   splineWaypoints, timeForTrajectory, samplePeriod);
            end

        else % Direct trajectory between waypoints
            timeForTrajectory = waypoints(i).timeForTrajectory;
            [setPointJointAngles, setPointTimes, failedPoints] = TaskSpaceTrajectorySetPoints(...
                waypoints(i-1), waypoints(i), timeForTrajectory, samplePeriod...
            );
        
            if failedPoints > 0
               % Try joint space cubic instead
               fprintf("taskSpaceStagesFromWaypoints: Direct cubic interpolation " +...
                    "failed in task space, trying again in joint space\n");
               [setPointJointAngles, setPointTimes] = JointAngleSetPointsStartEndUsingJointSpace(...
                waypoints(i-1).x, waypoints(i-1).y, waypoints(i-1).z, waypoints(i-1).theta,...
                waypoints(i).x, waypoints(i).y, waypoints(i).z, waypoints(i).theta,...
                timeForTrajectory, samplePeriod);
            end
        end

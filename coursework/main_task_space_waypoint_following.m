...
%     drawnow;
    ...
    elseif currentWaypoint <= length(waypoints) % Move along planned trajectories between waypoints
        ...
        % Plan task space trajectories and convert into joint space
        timeForTrajectory = 1; % In seconds
        samplePeriod = 0.1; % In seconds
        [jointAngles, times] = TaskSpaceTrajectorySetPoints(startPose, endPose, timeForTrajectory, samplePeriod);
        
        % Send trajectories to servos
        for i = 1:length(times)
            pos1 = radiansToEncoder(jointAngles(i).joint1_angle);
            pos2 = radiansToEncoder(jointAngles(i).joint2_angle);
            pos3 = radiansToEncoder(jointAngles(i).joint3_angle);
            pos4 = radiansToEncoder(jointAngles(i).joint4_angle);

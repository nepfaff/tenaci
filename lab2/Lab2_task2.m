...
while (j<length(sine))
    ...
    writePosition(DXL_ID1, goalPos1, port_num, PROTOCOL_VERSION, COMM_SUCCESS, ADDR_PRO_GOAL_POSITION)
    writePosition(DXL_ID2, goalPos2, port_num, PROTOCOL_VERSION, COMM_SUCCESS, ADDR_PRO_GOAL_POSITION)
    ...
    while(true)
       ...
       % Check if reached goal position
        if abs(pos1 - goalPos1) <= tolerance && abs(pos2 - goalPos2) <= tolerance
            break
        end 
    end
...
function sinWave = computeNegSinWave()
    step_size = 0.1;
    range = 500;
    translation = 2048;
